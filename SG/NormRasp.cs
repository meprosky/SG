...
namespace SG
{
    ...
    {
        //step 0.1, -3..3, //Разумов и др. Сетевые графики в планировании. Значения функции стандартного нормального закона распределения

        double x0 = -3.0;
        double nraStep = 0.1;
        double[] nra = {
            0.0013, 0.0019, 0.0026, 0.0035, 0.0047, 0.0062, 0.0082, 0.0107, 0.0139, 0.0179, //-3..-2.1
            0.0228, 0.0287, 0.0359, 0.0446, 0.0548, 0.0668, 0.0808, 0.0968, 0.1151, 0.1357, //-2..-1.1
            0.1587, 0.1841, 0.2119, 0.242,  0.2743, 0.3085, 0.3446, 0.3821, 0.4207, 0.4602, //-1..-0.1
            0.5,    0.5398, 0.5793, 0.6179, 0.6554, 0.6915, 0.7257, 0.758,  0.7881, 0.8159, //0..0.9
            0.8413, 0.8643, 0.8849, 0.9032, 0.9192, 0.9332, 0.9452, 0.9554, 0.9641, 0.9713, //1..1.9
            0.9772, 0.9821, 0.9861, 0.9893, 0.9916, 0.9938, 0.9953, 0.9965, 0.9974, 0.9981, 0.9987 //2..3
        };

            
        //step = 0.01, 0..3.49, //ГОСТ Р 50779.21-96 Значения функции стандартного нормального закона распределения
        double x0_2 = 0;
        double nraStep2 = 0.01;
        public double[] nra2 = { 
                     0.5, 0.50399, 0.50798, 0.51197, 0.51595, 0.51994, 0.52392, 0.5279,  0.53188, 0.53586,
                 0.53983, 0.5438,  0.54776, 0.55172, 0.55567, 0.55962, 0.56356, 0.5675,  0.57142, 0.57535,
                 ...
                 0.75804, 0.76115, 0.76424, 0.76731, 0.77035, 0.77337, 0.77637, 0.77935, 0.7823,  0.78524,
                 ...
                 0.84134, 0.84375, 0.84614, 0.8485,  0.85083, 0.85314, 0.85543, 0.85769, 0.85993, 0.86214,
        ...
        public double NormRasp(double Td, double Tk, double disp)
        {
            //double d1 = Tk - 3 * disp;
            //double d2 = Tk + 3 * disp;

            if (Td < Tk - 3 * disp) return 0.01;
            if (Td >= Tk + 3 * disp) return 0.99;

            double z = (Td - Tk) / disp;

            for (int i = 0; i < nra.Length - 1; i++)
            {
                double z1 = x0 + nraStep * i;
                double z2 = x0 + nraStep * (i + 1);
                
                if (z >= z1 && z <= z2)
                {
                    double interpolation = nra[i] + (z - z1) * (nra[i + 1] - nra[i]) / (z2 - z1);
        ...
        public double NormRasp2(double Td, double Tk, double disp)
        {
            //double d1 = Tk - 3 * disp;
            //double d2 = Tk + 3 * disp;
            ...
            for (int i = 0; i < nra2.Length - 1; i++)
            {
                double z1 = x0_2 + nraStep2 * i;
                double z2 = x0_2 + nraStep2 * (i + 1);

                if (z >= z1 && z <= z2)
                {
                    double interpolation = nra2[i] + (z - z1) * (nra2[i + 1] - nra2[i]) / (z2 - z1);
